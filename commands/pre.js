...
module.exports = {
    ...
    description: 'premiumlara özel hesap oluşturur.', // Command description
    ...
    execute(message, args) {
        ...
        try {
            message.client.channels.cache.get(config.preChannel).id; // Try to get the channel's id
        } catch (error) {
        ...
        if (message.channel.id === config.preChannel) {
            ...
            if (generated.has(message.author.id)) {
                ...
                const filePath = `${__dirname}/../premium/${args[0]}.txt`;
            ...
            message.channel.send(
                new MessageEmbed()
                    .setColor(config.color.red)
                    .setTitle('Hata!')
                    .setDescription(` \`pre\` komudunu kullanabilmek için  <#${config.preChannel}> kanalını kullanmalısınız`)
                    .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                    .setTimestamp()
            );
        };
