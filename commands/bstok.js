...
module.exports = {
  name: 'bstok', // Command name
  description: 'booster stok gösterir.', // Command description
  ...
  execute(message) {
    ...
    fs.readdir(`${__dirname}/../boost/`, function(err, files) {
      ...
      const embed = new MessageEmbed()
        ...
        .setTitle(`Boost stoğunda **${stock.length} tane çeşit var**`)
        .setDescription('')

      // Push all services to the stock
      stock.forEach(async function(data) {
        const acc = fs.readFileSync(`${__dirname}/../boost/${data}`, 'utf-8')
        const lines = [];
