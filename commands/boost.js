...
module.exports = {
    name: 'boost', // Command name
    description: 'booster lara özel hesap oluşturur.', // Command description
    ...
    execute(message, args) {
        ...
        try {
            message.client.channels.cache.get(config.boostChannel).id; // Try to get the channel's id
        } catch (error) {
        ...
        if (message.channel.id === config.boostChannel) {
            ...
            if (generated.has(message.author.id)) {
                ...
                const filePath = `${__dirname}/../boost/${args[0]}.txt`;
            ...
            message.channel.send(
                new MessageEmbed()
                    .setColor(config.color.red)
                    .setTitle('Hata!')
                    .setDescription(` \`boost\` komudunu kullanabilmek için  <#${config.boostChannel}> kanalını kullanmalısınız`)
                    .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                    .setTimestamp()
            );
        };
